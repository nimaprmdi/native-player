        ...
        <article className="w-full mb-8">
            ...
            <div className="c-content__meta w-full flex flex-wrap gap-3 justify-start items-center pl-4 mt-8 mb-8">
                ...
                {type === "track" && (
                    <div className="flex items-center mr-1 desktop:mr-6">
                        ...
                        <span className="text-gray-500 ml-1">Released in {postData.album && type === "track" ? postData.album.release_date : postData.release_date}</span>
                    </div>
                )}
            ...
            <div className="c-content__article w-full flex flex-wrap justify-center desktop:justify-start gap-2 relative">
                <div className="w-full flex flex-wrap desktop:flex-nowrap justify-start gap-2 px-2">
                    {postData.type === "track" && (
                        <>
                            {postData.preview_url && <SingleCards icon="ant-design:play-circle-filled" title="Play The Music" btnText="Play now" url="#" functional={() => handlePlay(postData.preview_url)} />}

                            <SingleCards icon="dashicons:album" title="go to album" btnText="go now" url={`/single?id=${postData.album?.id}&type=album`} />

                            <SingleCards icon="bi:person-lines-fill" title="go to artist" btnText="go now" url={`/single?id=${postData.artists![0].id}&type=artist`} />
                        </>
                    )}

                    {postData.type === "album" && (
                        <>
                            <SingleCards icon="dashicons:playlist-audio" title="Total tracks" btnText={postData.total_tracks.toString()} url="#" isLink={true} />
                    ...
                    {postData.type === "artist" && (
                        <>
                            <SingleCards icon="ic:baseline-category" title="Genres" btnText={postData.genres && postData.genres.length > 0 ? postData.genres?.map((genre) => genre).toString() : "no data found"} url={postData.external_urls?.spotify || ""} isLink={true} />

                            <SingleCards icon="fluent:people-audience-20-regular" title="followers" btnText={postData.followers?.total ? postData.followers.total.toString() : ""} url={postData.external_urls?.spotify || "no data found"} isLink={true} />
                        </>
                    )}

                    {postData.type === "playlist" && (
                        <>
                            <SingleCards icon="humbleicons:user-asking" title="Owner" btnText={postData.owner?.display_name ? postData.owner.display_name : " "} url={postData.external_urls?.spotify || ""} isLink={true} />

                            <SingleCards icon="fluent:people-audience-20-regular" title="followers" btnText={postData.followers?.total ? postData.followers.total.toString() : ""} url={postData.external_urls?.spotify || ""} isLink={true} />
                        </>
                    )}

                    <SingleCards icon="akar-icons:spotify-fill" title="Open in spotify" btnText="Open Now" url={postData.external_urls?.spotify || ""} isLink={true} />
                </div>
                    ...
                    <>
                            ...
                            <>
                                ...
                                <Carousel>
                                    {artistTracks.tracks.map((song, index) => {
                                        return <>{index < 8 && <CardPinkRibbon key={`recommended-artist-tracks-content-${song.id + Math.random() * 1000}`} id={song.id} image={song.album.images[0].url} name={song.name} type="track" />}</>;
                                    })}
                                </Carousel>
                            </>
                        )}
                        ...
                        <Carousel>
                            {recommendedTracks.tracks.map((track, index) => {
                                return <>{index < 8 && <CardPinkRibbon key={`recommended-tracks-content-${track.id + Math.random() * 1000}`} id={track.id} image={track.album.images[0].url} name={track.name} type="track" />}</>;
                            })}
                        </Carousel>
                    </>
                </div>
