...
const getPlayListsByCats = async (token: string, setPlayListsByCats: (data: SongCategory) => void) => {
  const topItems = await spotifyService.getPlayListsByCat(token);
  setPlayListsByCats(topItems.data);
};

const getFeaturedAlbums = async (token: string, setFeaturedAlbums: (data: FeaturedAlbums) => void) => {
  const featuredAlbums = await spotifyService.getFeaturedAlbums(token);
  setFeaturedAlbums(featuredAlbums.data);
};

const getFeaturedPlayList = async (token: string, setFeaturedPlayList: (data: FeaturedPlayLists) => void) => {
  const featuredPlayList = await spotifyService.getFeaturedPlayList(token);
  setFeaturedPlayList(featuredPlayList.data);
};

const getRelatedArtists = async (token: string, setRelatedArtists: (data: ArtitstsObj) => void) => {
