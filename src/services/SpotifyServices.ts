...
class SpotifyServices {
  ...
  getFeaturedPlayList = async (token: string) => {
    const { data, status } = await http.get<FeaturedPlayLists>(
      "/playlists/1bAxUEAiPtRRMsnHbkz7vP",
      {
  ...
  getRecommendedTracks = async (token: string) => {
    const { data, status } = await http.get<Recommendation>(
      "/recommendations",
      {
        headers: this.getHeaders(token),
        params: {
  ...
  getPlayListsByCat = async (token: string) => {
    const { data, status } = await http.get<SongCategory>(
  ...
  getRelatedArtists = async (token: string) => {
    const { data, status } = await http.get<ArtitstsObj>(
      "/artists/2CIMQHirSU0MQqyYHq0eOx/related-artists",
      {
        headers: this.getHeaders(token),
      }
    );
